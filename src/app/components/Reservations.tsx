...
const mockReservations = [
  { id: 1, name: "Eleanor Shellstrop", time: "18:30", pax: 2, table: "4", status: "confirmed", tags: ["Anniversary", "VIP"] },
  { id: 2, name: "Chidi Anagonye", time: "19:00", pax: 4, table: "12", status: "arrived", tags: ["Allergy: Nuts"] },
  { id: 3, name: "Tahani Al-Jamil", time: "20:00", pax: 6, table: "8", status: "pending", tags: ["VIP", "Birthday"] },
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-serif text-[#4A3B32] tracking-tight">Reservations</h1>
          <p className="text-[#8C7A6B] mt-1 text-sm">Manage bookings and waitlist</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center bg-white border border-[#E8E1D5] rounded-sm p-1 shadow-sm">
            ...
            <div className="px-4 py-1 flex items-center gap-2 border-x border-[#E8E1D5]">
              ...
              <span className="font-semibold text-[#4A3B32] text-xs uppercase tracking-widest">Today, Oct 24</span>
            </div>
            <button className="p-2 text-[#8C7A6B] hover:text-[#4A3B32] transition-colors rounded-sm hover:bg-[#F0EBE1]">
      ...
      <div className="grid lg:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <div className="lg:col-span-1 flex flex-col bg-white border border-[#E8E1D5] rounded-sm overflow-hidden shadow-sm">
          <div className="p-4 border-b border-[#E8E1D5] bg-[#FCFBF8]">
            ...
            <div className="flex gap-2 mt-4 overflow-x-auto pb-1 scrollbar-none">
              {["All", "Arrived", "Confirmed", "Waitlist"].map((t, i) => (
                <button key={t} className={`px-3 py-1.5 rounded-sm text-xs font-semibold tracking-wider uppercase whitespace-nowrap transition-colors ${i === 0 ? 'bg-[#4A3B32] text-[#D4AF37]' : 'bg-white border border-[#E8E1D5] text-[#8C7A6B] hover:text-[#4A3B32] hover:border-[#C4BCB3]'}`}>
                  {t}
                </button>
              ))}
            </div>
            ...
            {mockReservations.map((res, i) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-serif font-medium text-[#4A3B32] group-hover:text-[#D4AF37] transition-colors">{res.name}</h3>
                  <div className={`text-[10px] px-2 py-0.5 rounded-sm border font-semibold uppercase tracking-widest ${
                    res.status === 'arrived' ? 'bg-teal-50 text-teal-700 border-teal-200' :
                ...
                <div className="flex items-center gap-4 text-xs font-medium text-[#8C7A6B] mb-3">
                  <div className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-[#D4AF37]" /> {res.time}</div>
                  <div className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-[#D4AF37]" /> {res.pax} Pax</div>
                  <div className="text-[#4A3B32]">Table {res.table}</div>
                </div>

                {res.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {res.tags.map(tag => (
                      <span key={tag} className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 bg-[#D4AF37]/10 text-[#C5A028] border border-[#D4AF37]/30 rounded-sm flex items-center gap-1 font-semibold">
                        {tag.includes('VIP') ? <Star className="w-3 h-3" /> : null}
                        {tag.includes('Allergy') ? <Heart className="w-3 h-3" /> : null}
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <div className="lg:col-span-2 bg-white border border-[#E8E1D5] rounded-sm p-6 md:p-8 flex flex-col items-center justify-center text-center shadow-sm">
          ...
          <h2 className="text-2xl font-serif text-[#4A3B32] mb-2">Select a reservation</h2>
          <p className="text-[#8C7A6B] max-w-sm text-sm">
            Choose a reservation from the list to view details, assign tables, or modify booking information.
          </p>
          <div className="mt-8 pt-8 border-t border-[#E8E1D5] w-full max-w-md text-left">
            <h3 className="text-xs font-semibold text-[#8C7A6B] uppercase tracking-widest mb-4 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-[#D4AF37]" /> AI Suggestions
            </h3>
            <div className="bg-[#FCFBF8] border border-[#E8E1D5] rounded-sm p-4 relative overflow-hidden">
              ...
              <p className="text-sm text-[#4A3B32]">
                You have an unassigned party of 6 (Tahani Al-Jamil) at 20:00. <span className="font-semibold">Table 8</span> is available and perfect for VIP groups of this size.
              </p>
